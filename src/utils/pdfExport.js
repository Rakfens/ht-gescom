...
export const generateClientPDF = (client, livraisons, recuperation, province, logoUrlParam) => {
  // Regrouper par client donneur (déjà fait dans l'appel)
  ...
  <body>
    ...
    <div class="total-box">
      <div class="total-row">
        ...
        <span>${formatAr(totalMontant)}</span>
      </div>
      ...
      <div class="total-row net-row">
        ...
        <span>${formatAr(net)}</span>
      </div>
    </div>
    
    <div class="footer">
      Aterinay Services — Merci pour votre confiance
    </div>
...
export const printAgentList = (agent, livraisons, date, logoUrlParam) => {
  ...
  for (const l of livraisons) {
    ...
    clientsMap[client].livraisons.push(l);
    clientsMap[client].totalMontant += montant;
    clientsMap[client].totalFrais += frais;
  }
  ...
  for (const client in clientsMap) {
    ...
    const totalClient = data.totalMontant + data.totalFrais;
    ...
    clientsHtml += `<div class="total-line grand">💵 TOTAL CLIENT : ${formatAr(totalClient)}</div>`;
    clientsHtml += `</div></div>`;
    clientNum++;
  }
  ...
  <head>
    ...
    <style>
      * { margin: 0; padding: 0; box-sizing: border-box; }
      ...
      .liv-title { font-weight: bold; font-size: 10px; margin-bottom: 2px; }
  ...
  <body>
    ...
    <div class="grand-total">
      ...
      <div class="gt-total gt-row"><span>💵 TOTAL À REMETTRE :</span><span>${formatAr(grandTotalMontant + grandTotalFrais)}</span></div>
    </div>
