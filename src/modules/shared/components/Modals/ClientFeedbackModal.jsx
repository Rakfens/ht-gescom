// modules/shared/components/Modals/ClientFeedbackModal.jsx
...
export const ClientFeedbackModal = ({ 
  ...
  const aVerser = totalMontantFacture - fbRecup - fbProvince;
  ...
  const handleGeneratePDF = () => {
    generateClientPDF(
      ...
      fbProvince, 
      logoUrl,
      currentCompany // Passer la société pour le PDF
    );
    setFbClient(null);
  };
    ...
    <div style={{ 
      ...
      <div style={{ 
        ...
        <div style={{ fontWeight: 800, color: '#f1f5f9', fontSize: 18, marginBottom: 4 }}>
          📄 Feedback — {fbClient.client}
        </div>
        <div style={{ color: COLORS.muted, fontSize: 12, marginBottom: 18 }}>
          {fbClient.livs.length} livraison(s) — {histDate}
          {currentCompany && (
            <span style={{ marginLeft: 8, padding: '2px 6px', background: COLORS.border, borderRadius: 4, fontSize: 10 }}>
              {currentCompany.name}
            </span>
          )}
        </div>
        ...
        <div style={{ marginBottom: 12 }}>
          ...
          <input 
            ...
            placeholder="0" 
            value={fbRecup} 
            onChange={e => setFbRecup(parseFloat(e.target.value) || 0)} 
          />
        </div>
        <div style={{ marginBottom: 20 }}>
          ...
          <input 
            ...
            placeholder="0" 
            value={fbProvince} 
            onChange={e => setFbProvince(parseFloat(e.target.value) || 0)} 
          />
        </div>
        ...
        <div style={{ background: COLORS.bg, borderRadius: 10, padding: '12px 16px', marginBottom: 20 }}>
          ...
          <div style={{ 
            ...
            <span>À VERSER</span>
            <span style={{ color: COLORS.green }}>{formatAr(aVerser)}</span>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          <button 
            style={{ ...btn('#475569', '#334155'), padding: 12 }} 
            onClick={() => setFbClient(null)}
          >
            ✕
          </button>
        </div>
