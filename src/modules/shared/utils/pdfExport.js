// modules/shared/utils/pdfExport.js
import { formatAr, STATUTS } from './constants';
import { getCurrentCompany } from '../../../supabaseClient';  // ← CHEMIN CORRIGÉ
import { supabase } from '../../../supabaseClient';   

/**
 * Récupère le logo de la société active
 * @param {string} logoUrlParam - URL du logo passé en paramètre (optionnel)
 * @returns {Promise<string>} URL du logo
 */
const getCompanyLogo = async (logoUrlParam = null) => {
  // Si un logo est passé en paramètre, l'utiliser
  if (logoUrlParam) return logoUrlParam;
  
  // Essayer de récupérer le logo depuis la société active
  const company = getCurrentCompany();
  if (company?.logo_url) return company.logo_url;
  
  // Sinon, essayer depuis la table config (compatibilité ancien système)
  try {
...
/**
 * Récupère le nom de la société active
 * @returns {string} Nom de la société
 */
const getCompanyName = () => {
  const company = getCurrentCompany();
  return company?.name || 'Aterinay Services';
};

/**
 * Génère un PDF de facture client (version multi-sociétés)
 * @param {string} client - Nom du client
 * @param {Array} livraisons - Liste des livraisons
 * @param {number} recuperation - Montant récupération
 * @param {number} province - Montant province
 * @param {string} logoUrlParam - URL du logo (optionnel)
 */
export const generateClientPDF = async (client, livraisons, recuperation, province, logoUrlParam = null) => {
  const date = new Date().toISOString().split('T')[0];
  const w = window.open('', '_blank');
  if (!w) { 
    alert('Autorisez les popups'); 
    return; 
  }
  
  const logoUrl = await getCompanyLogo(logoUrlParam);
  const companyName = getCompanyName();
  const companyType = getCurrentCompany()?.type === 'service' ? 'Service de livraison' : 'Boutique';
  ...
  <body>
    <div class="header">
      ...
      <div class="subtitle">${companyType} — Relevé de livraisons</div>
    </div>
    ...
    <div class="footer">${companyName} — Merci pour votre confiance</div>
...
/**
 * Génère une liste imprimable pour un agent (livreur)
 * @param {Object} agent - Agent/livreur
 * @param {Array} livraisons - Liste des livraisons
 * @param {string} date - Date
 * @param {string} logoUrlParam - URL du logo (optionnel)
 */
export const printAgentList = async (agent, livraisons, date, logoUrlParam = null) => {
  const w = window.open('', '_blank');
  if (!w) return;
  ...
  <head>
    <meta charset="UTF-8">
    <title>Rapport livreur - ${agent.nom} - ${companyName}</title>
    <style>
      *{margin:0;padding:0;box-sizing:border-box}
      body{
